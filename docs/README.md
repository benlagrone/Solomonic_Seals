# Solomonic Seals Docs Index

This folder contains product, structure, and source-indexing docs for Solomonic Seals.

Program-level strategy, deployment policy, and release governance are maintained in:
- [Fortress Phronesis Canonical Docs Index](../../fortress-phronesis/docs/README.md)

## Shared Acceptance + Demo

- [v1.1.3 Acceptance Test + Demo Guide](../../fortress-phronesis/docs/acceptance-demo-guide-v1.1.3.md)

## Local Documents

- [Clock Contract Extraction Inventory](clock_contract_inventory.md) — observed domain, data, runtime, Pericope and UI dependencies
- [SolomonicSchedule v1](solomonic_schedule_v1.md) — versioned solver-independent display contract and production compatibility gates; documentation-only release, no runtime replacement
- [Combined Roadmap](combined_roadmap.md) — canonical execution order across product, mobile, scripture, runtime, Pericope, and Life OS work
- [Clock Content Enrichment Feature Specification](clock_content_enrichment_feature_spec.md) — concrete fixed-now content, existing-section meaning, work citations, manuscript treatment, personalization, and additive Clock API features with acceptance criteria
- [Harmonic Time-Scale Content Plan](harmonic_time_scale_content_plan.md) — present-moment architecture from minute through era, harmonic resolution, provenance, content production, and delivery stages
- [Roadmap](roadmap.md)
- [Startup Notes](startup.md)
- [Instructions](instructions.md)
- [Intent Reading Map](intent_reading_map.md)
- [Product One Pager](product_one_pager.md)
- [Marketing Name Candidates](marketing_name_candidates.md)
- [Deployment Promotion Runbook](deployment_promotion_runbook.md)
- [Production Domain Setup](production_domain_setup.md)
- [MVP Mobile App Roadmap](mvp_mobile_app_roadmap.md)
- [Paid Access Test Plan](paid_access_test_plan.md) — test-mode entitlement roles, enforcement modes, first protected feature, and promotion gates
- [Combined Roadmap](combined_roadmap.md)
- [Solomonic Calendar Expansion Guide](solomonic_calendar_expansion_guide.md)
- [Source Texts Indexing Plan](source_texts_indexing_plan.md)
- [Pericope Psalter Handoff](pericope_psalter_handoff.md)
- [Clock Contract For PericopeAI](clock_pericope_contract.md)
- [Clock Consumption Contract](clock_consumption_contract.md)
- [Ask Proverbs Boundary Contract](ask_proverbs_boundary_contract.md)
- [Hierarchy Diagram](hierarchy.mmd)
- [Clock Hierarchy Diagram](clock-hierachy.mmd)
- [126 Seals Diagram](126-seals.mmd)
- [44 Planetary Diagram](44-planetary.mmd)
- [Source Texts Folder](source_texts/)

## Scope

- Keep Solomonic-specific feature plans and source-text structures here.
- Put cross-project deployment/process changes in `fortress-phronesis/docs`.
